...
// This ensures Firestore paths are constructed correctly.
...
try {
    ...
    logErrorToFirestore("agreementCustomDataInit", e.message, e); // This might fail if Firestore isn't ready or rules deny
    agreementCustomData = {
...
let selectedWorkerEmailForAuth = null; // Used to store the key (email or UID) of the worker being authorized
...
async function logErrorToFirestore(location, errorMsg, errorDetails = {}) {
    ...
    try {
        ...
        await fsAddDoc(errorLogRef, {
            ...
            appVersion: "1.0.4", // Consider making this dynamic
...
async function initializeFirebase() {
    console.log("Attempting to initialize Firebase with config from window.firebaseConfigForApp");
        ...
        onAuthStateChanged(fbAuth, async (user) => {
            ...
            try {
                if (user) {
                    ...
                    if (userProfileData) { // Existing user with a profile
                    ...
                    } else if (currentUserId && !userProfileData) { // User in Auth, but no Firestore profile (treat as new registration completion)
                        console.log(`User ${currentUserEmail || currentUserId} authenticated but no profile found in Firestore. Proceeding with new profile creation.`);
                        const isOrgPortal = globalSettings.portalType === 'organization';
                        profile = {
                            name: currentUserEmail ? currentUserEmail.split('@')[0] : 'New User',
                            email: currentUserEmail || null,
                            uid: currentUserId,
                            isAdmin: false,
                            abn: "", gstRegistered: false, bsb: "", acc: "", files: [],
                        ...
                        try {
                            await setDoc(userProfileDocRef, profile);
                            console.log(`New profile CREATED in Firestore for ${currentUserEmail || currentUserId} with approved: ${profile.approved}`);
                            accounts[currentUserEmail || currentUserId] = { name: profile.name, profile: profile };

                            if (isOrgPortal && !profile.approved) { // This will be true if it's an org portal
                            ...
                            // For non-org portals or if somehow approved (e.g., if default portalType was 'participant')
                            await loadAllDataForUser();
                            if (globalSettings.portalType === 'organization' && !profile.profileSetupComplete) {
                                 openUserSetupWizard(); // Should ideally not be hit if just logged out due to approval
                            } else {
                                enterPortal(false);
                            }
                        } catch (profileCreationError) {
                            console.error("CRITICAL: Failed to create new user profile in Firestore during onAuthStateChanged:", profileCreationError);
                            logErrorToFirestore("onAuthStateChanged_newProfileCreation", profileCreationError.message, profileCreationError);
                            showMessage("Registration Finalization Error", "Could not save your profile information. Please contact support.");
                            await fbSignOut(fbAuth); // Log out if profile creation fails to prevent inconsistent state
                            hideLoading();
                            return;
                        }
                    } else { // User signed out OR an unexpected state
                        console.warn("User is signed out or in an unexpected state in onAuthStateChanged.");
...
async function loadUserProfileFromFirestore(userIdToLoad) {
    ...
    try {
        ...
        if (docSnap.exists()) {
            console.log(`Profile loaded for ${userIdToLoad}:`, docSnap.data());
            return docSnap.data();
        } else {
            console.log(`No profile document found for ${userIdToLoad} at path ${userProfileDocRef.path}`);
            return null;
        }
    } catch (error) {
...
async function saveAdminServiceToFirestore(servicePayload, serviceIdToUpdate = null) {
    ...
    try {
        ...
        if (serviceIdToUpdate) { 
            ...
            payloadWithAudit = {
                ...servicePayload, id: serviceDocRef.id, 
                lastUpdated: serverTimestamp(), updatedBy: currentUserEmail || currentUserId
            };
        } else { 
            ...
            payloadWithAudit = {
                ...servicePayload, id: serviceDocRef.id, 
                createdAt: serverTimestamp(), createdBy: currentUserEmail || currentUserId,
                lastUpdated: serverTimestamp(), updatedBy: currentUserEmail || currentUserId
            };
        }
        ...
        if (docSnap.exists()) {
            ...
            if (loadedData.clauses && Array.isArray(loadedData.clauses) && loadedData.clauses.length > 0) {
             ...
             agreementCustomData = { 
                ...
                createdAt: serverTimestamp(), createdBy: "system_init" 
            };
    ...
    try {
        ...
        if (userCredential && userCredential.user) {
            ...
            const initialProfileData = {
                name: email.split('@')[0], 
                email: email, uid: newUserId, isAdmin: false, abn: "", gstRegistered: false,
                bsb: "", acc: "", files: [], authorizedServiceCodes: [],
                profileSetupComplete: false, nextInvoiceNumber: 1001,
            ...
            try {
                ...
                console.log(`Profile document CREATED for new user ${email} during registration. Approved: ${initialProfileData.approved}`);
                // onAuthStateChanged will handle the next steps.
                // Provide immediate feedback based on approval status.
                if (isOrgPortal && !initialProfileData.approved) {
                    showMessage("Registration Successful", "Your account has been created and is awaiting administrator approval. You will be logged out shortly by the system.");
                } else { 
                    showMessage("Registration Successful", "Your account has been created! You will be logged in shortly.");
                }
            } catch (profileError) {
                console.error("CRITICAL: Failed to create Firestore profile during registration:", profileError);
                logErrorToFirestore("modalRegister_profileCreation", profileError.message, { email, profileError });
                showAuthStatusMessage("Registration Error: Could not save your profile. Please contact support. Your Auth account was created but profile setup failed.");
                // Consider if the Auth user should be deleted here or flagged for admin.
                // For now, user will exist in Auth but might have issues if onAuthStateChanged can't create profile later.
            }
        }
    } catch (err) {
    ...
    for (const file of filesToUpload) {
        ...
        uploadPromises.push(
            uploadBytes(storageRef, file).then(async (snapshot) => {
                ...
                newFileEntries.push({
                    name: file.name, 
                    url: downloadURL, storagePath: snapshot.ref.fullPath, 
                    uploadedAt: serverTimestamp(), size: file.size, type: file.type
                });
    ...
    try {
        ...
        if (newFileEntries.length > 0) {
            ...
            await updateDoc(userProfileDocRef, {
                ...
                lastUpdated: serverTimestamp(), updatedBy: currentUserId
            });
...
window.generateInvoicePdf = function() {
    ...
    rows.forEach((row) => {
        ...
        currentInvoiceData.items.push({
            date: itemDateEl ? itemDateEl.value : "",
            serviceCode: serviceCode, description: service ? service.description : "N/A",
            startTime: itemStartTimeEl ? itemStartTimeEl.dataset.value24 : "",
            endTime: itemEndTimeEl ? itemEndTimeEl.dataset.value24 : "",
            hoursOrKm: parseFloat(row.cells[8].textContent) || 0,
            total: parseFloat(row.cells[10].textContent.replace('$', '')) || 0,
    ...
    let pdfHtml = `
        <style>
            body { font-family: 'Inter', sans-serif; font-size: 10pt; } .pdf-invoice-container { padding: 15mm; }
            .pdf-header { text-align: center; margin-bottom: 15mm; } .pdf-header h1 { margin: 0 0 3mm 0; font-size: 20pt; color: #333; }
            ...
            .pdf-details-grid div { margin-bottom: 2mm; } .pdf-details-grid strong { font-weight: 600; }
            ...
            .pdf-table th { background-color: #e9ecef; font-weight: 600; } .pdf-table tr { page-break-inside: avoid !important; }
            ...
            .pdf-totals div { display: flex; justify-content: space-between; margin-bottom: 3mm; } .pdf-totals strong { font-weight: 600; }
        </style>
        <div class="pdf-invoice-container">
            <div class="pdf-header"><h1>Tax Invoice</h1><p>${currentInvoiceData.providerName ?? "Provider Name N/A"}</p><p>ABN: ${currentInvoiceData.providerAbn ?? "ABN N/A"}</p>${(currentInvoiceData.gstRegistered) ? '<p>GST Registered</p>' : ''}</div>
            <div class="pdf-details-grid"><div><strong>To:</strong> ${globalSettings.participantName ?? 'Participant N/A'}<br>NDIS No: ${globalSettings.participantNdisNo ?? 'N/A'}<br>${globalSettings.planManagerName ? `Plan Manager: ${globalSettings.planManagerName}<br>` : ''}${globalSettings.planManagerEmail ? `Email: ${globalSettings.planManagerEmail}<br>` : ''}</div><div><strong>Invoice #:</strong> ${currentInvoiceData.invoiceNumber || 'N/A'}<br><strong>Date Issued:</strong> ${formatDateForInvoiceDisplay(currentInvoiceData.invoiceDate) || 'N/A'}<br><strong>Support Worker:</strong> ${profile.name ?? 'N/A'}</div></div>
            <table class="pdf-table"><thead><tr><th>Date</th><th>NDIS Code</th><th>Description</th><th>Start</th><th>End</th><th>Rate Type</th><th class="number">Rate/Unit ($)</th><th class="number">Hours/Km</th><th class="number">Total ($)</th></tr></thead><tbody>`;
    ...
    currentInvoiceData.items.forEach(item => {
        ...
        if (service && service.rates) {
            if (service.categoryType === SERVICE_CATEGORY_TYPES.TRAVEL_KM) {
                rateForPdf = service.rates.perKmRate || 0; rateTypeForPdf = "Travel"; 
        ...
        pdfHtml += `<tr><td>${formatDateForInvoiceDisplay(item.date) ?? 'N/A'}</td><td>${item.serviceCode ?? 'N/A'}</td><td>${item.description ?? 'N/A'}</td><td>${formatTime12Hour(item.startTime) ?? 'N/A'}</td><td>${formatTime12Hour(item.endTime) ?? 'N/A'}</td><td>${rateTypeForPdf ?? 'N/A'}</td><td class="number">${rateForPdf.toFixed(2)}</td><td class="number">${(item.hoursOrKm ?? 0).toFixed(2)}</td><td class="number">${(item.total ?? 0).toFixed(2)}</td></tr>`;
    });

    pdfHtml += `</tbody></table><div class="pdf-totals"><div><span>Subtotal:</span> <strong>$${(currentInvoiceData.subtotal ?? 0).toFixed(2)}</strong></div>`;
    if (currentInvoiceData.gstRegistered) { 
        pdfHtml += `<div><span>GST (10%):</span> <strong>$${(currentInvoiceData.gst ?? 0).toFixed(2)}</strong></div>`;
    }
    pdfHtml += `   <div><span>Total:</span> <strong>$${(currentInvoiceData.grandTotal ?? 0).toFixed(2)}</strong></div></div></div>`;
...
window.saveSig = async function() {
    if (!canvas || !ctx) {
        showMessage("Error", "Signature pad not ready."); closeModal('sigModal'); return;
    }
    if (isCanvasBlank(canvas)) {
        showMessage("Signature Required", "Please draw your signature before saving."); return;
    }
    ...
    if (!currentUserId || !fsDb) {
        showMessage("Error", "Cannot save signature. User or database not ready."); closeModal('sigModal'); return;
    }
    ...
    if (profile.isAdmin && currentAgreementWorkerEmail) { 
        ...
        if (!workerProfileForAgreement) {
            hideLoading(); showMessage("Error", "Selected worker profile not found for agreement."); closeModal('sigModal'); return;
        }
        ...
        hideLoading(); showMessage("Error", "Cannot determine whose agreement to update."); closeModal('sigModal'); return;
    }

    const updateData = {};
    if (signingAs === 'worker') {
        updateData.workerSigUrl = signatureDataUrl; updateData.workerSignDate = serverTimestamp(); updateData.workerSigned = true;
    } else if (signingAs === 'participant') { 
        updateData.participantSigUrl = signatureDataUrl; updateData.participantSignDate = serverTimestamp(); updateData.participantSigned = true;
    } else {
        hideLoading(); showMessage("Error", "Invalid signing role."); closeModal('sigModal'); return;
    }
    ...
    try {
        ...
        hideLoading(); closeModal('sigModal');
    }
};
...
function openUserSetupWizard(isEditing = false) {
    ...
    if (wizModal) {
        ...
        wizModal.classList.remove('hide'); wizModal.style.display = "flex";
        if (!isEditing) { 
...
window.wizNext = function() {
    if (userWizStep === 1) { 
        ...
        let bsb = $("#wBsb")?.value.trim(); let acc = $("#wAcc")?.value.trim();
        if (bsb) bsb = bsb.replace(/\D/g, ''); if (acc) acc = acc.replace(/\D/g, ''); 
    ...
    if (userWizStep < 4) { 
        userWizStep++; updateUserWizardView();
    }
};
window.wizPrev = function() {
    if (userWizStep > 1) { userWizStep--; updateUserWizardView(); }
};
window.wizFinish = async function() {
    if (!currentUserId || !fsDb) {
        showMessage("Error", "Cannot save profile. User not logged in or database not ready."); return;
    }
    ...
    const profileUpdates = {
        name: nameValue, abn: abnValue || profile.abn || "", 
        gstRegistered: $("#wGst")?.checked || false,
        bsb: bsbValue || profile.bsb || "", acc: accValue || profile.acc || "",
        profileSetupComplete: true, 
        lastUpdated: serverTimestamp(), updatedBy: currentUserId
    };
     if (!profile.createdAt) { 
        profileUpdates.createdAt = serverTimestamp(); profileUpdates.createdBy = currentUserId;
    }
    ...
    try {
        ...
        if (accounts[currentUserEmail]) { accounts[currentUserEmail].profile = profile; } 
        else if (accounts[currentUserId]) { accounts[currentUserId].profile = profile; }
        hideLoading(); closeModal('wiz');
        ...
        hideLoading(); console.error("Error saving profile from wizard:", error);
...
window.saveRequest = async function() {
    if (!currentUserId || !fsDb) {
        showMessage("Error", "Cannot save request. User not logged in or database not ready."); return;
    }
    ...
    const requestData = {
        userId: currentUserId, userName: profile.name || currentUserEmail, 
        date: requestDate, startTime: requestStartTime, endTime: requestEndTime,
        reason: requestReason || "", status: "pending", 
        requestedAt: serverTimestamp(), requestedBy: currentUserId
    };
    ...
    try {
        ...
        hideLoading(); closeModal('rqModal');
        ...
        hideLoading(); console.error("Error submitting shift request:", error);
...
window.saveInitialInvoiceNumber = async function() {
    if (!currentUserId || !fsDb || !profile) {
        showMessage("Error", "User not logged in or profile not ready."); return;
    }
    ...
    try {
        ...
        if (location.hash === "#invoice") { $("#invNo").value = formatInvoiceNumber(initialNumber); }
        hideLoading(); closeModal('setInitialInvoiceModal');
        showMessage("Invoice Number Set", `Starting invoice number set to ${initialNumber}.`);
    } catch (error) {
        hideLoading(); console.error("Error saving initial invoice number:", error);
...
window.saveShiftFromModalToInvoice = function() {
    ...
    if (!profile.isAdmin && !(profile.authorizedServiceCodes?.includes(supportTypeCode))) { 
    ...
    if (claimTravel) {
        ...
        if (calculatedKm <= 0) {
            ...
            if (travelService) {
                addInvoiceRow({ date: shiftDate, serviceCode: travelService.code, travelKmInput: calculatedKm, claimTravel: true });
            } else {
    ...
    calculateInvoiceTotals(); closeModal('logShiftModal');
...
window.adminWizNext = function() {
    ...
    } else { console.log("Already on the last step of admin wizard."); }
};
window.adminWizPrev = function() {
    if (adminWizStep > 1) { adminWizStep--; updateAdminWizardView(); }
};
window.adminWizFinish = async function() {
    if (!isFirebaseInitialized || !(profile && profile.isAdmin)) {
        showMessage("Error", "Permission denied or system not ready."); return;
    }
    ...
    if (!portalTypeSelected) { 
        ...
        adminWizStep = 1; updateAdminWizardView(); return;
    }
    ...
    let tempGlobalSettings = { 
        ...
        setupComplete: true, lastUpdated: serverTimestamp(), 
    ...
    if (portalTypeSelected === 'organization') {
        ...
        tempGlobalSettings.organizationAbn = $("#adminWizOrgAbn")?.value.trim().replace(/\D/g, '') || ""; 
    ...
    try {
        ...
        hideLoading(); closeModal('adminSetupWizard');
        ...
        hideLoading(); console.error("Error finalizing admin setup:", error);
...
async function loadAllUserAccountsForAdminFromFirestore() {
    if (!isFirebaseInitialized || !(profile && profile.isAdmin)) {
        console.warn("Admin data load skipped: Firebase not ready or user not admin."); return;
    }
    showLoading("Loading user accounts...");
    try {
        const usersCollectionRef = collection(fsDb, `artifacts/${appId}/users`);
        const usersSnapshot = await getDocs(usersCollectionRef);
        const tempAccounts = {}; pendingApprovalAccounts = []; 
            ...
            profilePromises.push(
                getDoc(userProfileDocRef).then(profileSnap => {
                    if (profileSnap.exists()) {
                        ...
                        if (userData.email) { tempAccounts[userData.email] = userAccount; } 
                        else { tempAccounts[userId] = userAccount; }
...
function displayPendingWorkersForAdmin() {
    ...
    if (!ul) {
        console.warn("Element #pendingWorkersList not found for displaying pending workers."); return;
    }
    ul.innerHTML = ""; 
    if (pendingApprovalAccounts.length === 0) {
        ul.innerHTML = "<li>No workers currently awaiting approval.</li>"; return;
    }
    pendingApprovalAccounts.forEach(worker => {
        const li = document.createElement("li"); li.dataset.uid = worker.uid;
        const icon = document.createElement("i"); icon.className = "fas fa-user-clock"; li.appendChild(icon);
        li.appendChild(document.createTextNode(` ${worker.name || 'Unnamed Worker'} `));
        const small = document.createElement("small"); small.textContent = `(${worker.email || worker.uid})`; li.appendChild(small);
        const approveButton = document.createElement("button");
        approveButton.className = "btn-ok btn-small"; approveButton.title = "Approve Worker";
        approveButton.innerHTML = '<i class="fas fa-check"></i> Approve'; 
        approveButton.addEventListener('click', () => { // Programmatic listener
            if (typeof window.approveWorkerInFirestore === 'function') {
                window.approveWorkerInFirestore(worker.uid);
            } else {
                console.error("approveWorkerInFirestore is not a function when button clicked.");
                showMessage("Critical Error", "Approval function is not available.");
...
// Ensure approveWorkerInFirestore is globally accessible if called from dynamically generated HTML or addEventListener
window.approveWorkerInFirestore = async function(workerId) {
    console.log(`[ADMIN APPROVAL CALLED] workerId received: ${workerId}`); 
    if (!isFirebaseInitialized || !fsDb || !(profile && profile.isAdmin)) {
        showMessage("Error", "Cannot approve worker. System not ready or insufficient permissions."); return;
    }
    if (!workerId) { 
        showMessage("Error", "Worker ID not provided for approval."); return;
    }
    showLoading(`Approving worker ${workerId}...`);
    try {
        const workerProfileDocRef = doc(fsDb, `artifacts/${appId}/users/${workerId}/profile`, "details");
        // Check if document exists before trying to update. This might be redundant if profile creation is robust.
        const profileSnap = await getDoc(workerProfileDocRef);
        if (!profileSnap.exists()) {
            console.error(`[ADMIN APPROVAL] Profile document does NOT exist for workerId: ${workerId} at path ${workerProfileDocRef.path}. Cannot approve.`);
            logErrorToFirestore("approveWorkerInFirestore_noDocToUpdate", `Profile doc missing for ${workerId}`, { workerId });
            showMessage("Error", `Cannot approve worker: Profile data missing. User might need to attempt login again to trigger profile creation if it failed.`);
            hideLoading();
            return;
        }

        console.log(`[ADMIN APPROVAL] Attempting to update Firestore for path: ${workerProfileDocRef.path}`); 
        await updateDoc(workerProfileDocRef, {
            approved: true, lastUpdated: serverTimestamp(), updatedBy: currentUserId 
        });
        console.log(`[ADMIN APPROVAL] Successfully updated Firestore for workerId: ${workerId}. 'approved' should now be true.`); 
        ...
        } else if (accounts[workerId]?.profile) { 
        ...
        console.error(`[ADMIN APPROVAL] Firestore update FAILED for workerId: ${workerId}`, error); 
...
function displayWorkersForAuth() {
    ...
    if (!ul) {
        console.error("CRITICAL: UI Element #workersListForAuth not found in the HTML.");
        const adminWorkerPanel = $("#adminWorkerManagement"); 
        if (adminWorkerPanel && !adminWorkerPanel.querySelector('#workersListForAuthError')) { 
            const errorP = document.createElement('p'); errorP.id = "workersListForAuthError";
            errorP.style.color = "red";
            errorP.innerHTML = "Error: UI component <code>#workersListForAuth</code> is missing. Worker list cannot be displayed.";
            if (adminWorkerPanel.firstChild) { adminWorkerPanel.insertBefore(errorP, adminWorkerPanel.firstChild); } 
            else { adminWorkerPanel.appendChild(errorP); }
        }
        return;
    }
    ...
    const workerAccounts = Object.entries(accounts).filter(([key, acc]) => {
        return acc && acc.profile && !acc.profile.isAdmin && acc.profile.uid !== currentUserId && acc.profile.approved === true; 
    });
    ...
    workerAccounts.forEach(([key, worker]) => {
        ...
        li.dataset.key = key; 
        li.addEventListener('click', () => window.selectWorkerForAuth(key)); // Use addEventListener and ensure selectWorkerForAuth is global
        ul.appendChild(li);
    });
}

// Ensure selectWorkerForAuth is globally accessible
window.selectWorkerForAuth = function(key) { 
...
function displayServicesForWorkerAuth(workerProfileData) {
    ...
    if (adminManagedServices.length === 0) {
        ul.innerHTML = "<li>No NDIS services have been defined by the admin yet.</li>"; return;
    }
    ...
    adminManagedServices.forEach(service => {
        if (service.categoryType !== SERVICE_CATEGORY_TYPES.TRAVEL_KM) { 
            ...
            checkbox.value = service.code; checkbox.checked = authorizedCodes.includes(service.code); 
...
async function saveWorkerAuthorizationsToFirestore() {
    if (!isFirebaseInitialized || !selectedWorkerEmailForAuth || !accounts[selectedWorkerEmailForAuth]?.profile) {
        showMessage("Error", "No worker selected or worker data is invalid. Cannot save authorizations."); return;
    }
    const workerUid = accounts[selectedWorkerEmailForAuth].profile.uid; 
    if (!workerUid) { 
        showMessage("Error", "Worker User ID not found. Cannot save authorizations."); return;
    }
    ...
    try {
        ...
        hideLoading(); console.error("Error saving worker authorizations to Firestore:", e);
...
function setActive(hash) {
  ...
  $$("nav#side a.link, nav#bottom a.bLink").forEach(a => {
      if (a.hash === "#home") { a.classList.remove('hide');
          ...
          if (profile && profile.isAdmin) a.classList.remove('hide'); else a.classList.add('hide');
  ...
  if (currentHash === "#invoice" && !(profile && profile.isAdmin)) handleInvoicePageLoad();
    ...
    if (!activeAdminTab && $$('.admin-tab-btn').length > 0) { 
        ...
        if (targetAdminPanelId === "adminInvoiceCustomization") { /* Future logic */ }
    }
...
function loadAdminInvoiceCustomizations() {
    ...
    if (container && (!container.innerHTML.trim() || container.innerHTML.includes("will be available here"))) {
        container.innerHTML = `<p>Invoice template customization options will be available here in a future update.</p><ul><li>Upload a logo for invoices.</li><li>Define default payment terms or bank details.</li><li>Customize footer text.</li></ul>`;
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (loginBtn)    loginBtn.addEventListener("click", window.modalLogin); // Ensure modalLogin is on window
    if (registerBtn) registerBtn.addEventListener("click", window.modalRegister); // Ensure modalRegister is on window
    ...
    if (selectTravelCodeBtnEl) selectTravelCodeBtnEl.addEventListener('click', () => {
        ...
        if (travelServices.length === 0) {
            ...
            const ul = document.createElement('ul'); ul.className = 'modal-selectable-list';
            travelServices.forEach(s => {
                const li = document.createElement('li'); li.textContent = `${s.description} (${s.code})`;
                li.dataset.code = s.code; li.dataset.description = s.description;
                li.onclick = () => { $$('#travelCodeListContainer li').forEach(item => item.classList.remove('selected')); li.classList.add('selected'); };
                ul.appendChild(li);
            });
        ...
        if(travelModal) { travelModal.classList.remove('hide'); travelModal.style.display = "flex"; }
    });
    ...
    if (confirmTravelCodeBtn) confirmTravelCodeBtn.addEventListener('click', () => {
        ...
        if (selectedLi) {
            const code = selectedLi.dataset.code; const desc = selectedLi.dataset.description;
        ...
        } else { showMessage("Selection Error", "Please select a travel code from the list or cancel."); }
    });

    const timePickerBackBtn = $("#timePickerBackButton");
    if (timePickerBackBtn) timePickerBackBtn.addEventListener('click', ()=>{
        if(currentTimePickerStep==='minute'){ selectedMinute=null; $$('#timePickerMinutes button').forEach(b=>b.classList.remove('selected')); currentTimePickerStep='hour'; } 
        else if(currentTimePickerStep==='hour'){ selectedHour12=null; $$('#timePickerHours button').forEach(b=>b.classList.remove('selected')); currentTimePickerStep='ampm'; }
    ...
    if (setTimeBtnEl) setTimeBtnEl.addEventListener('click', ()=>{
        if(activeTimeInput && selectedAmPm!=null && selectedHour12!=null && selectedMinute!=null){
            ...
            activeTimeInput.value=timeString12; activeTimeInput.dataset.value24=timeString24; 
    ...
    if (agreementPdfBtn) agreementPdfBtn.addEventListener('click', () => { generateAgreementPdf(); });
    ...
    if (canvas) {
        ...
        } else { console.error("Could not get 2D context for signature canvas."); }
    } else { console.warn("Signature canvas element #signatureCanvas not found."); }
...
function handleAddAgreementClause() {
    const clausesContainer = $("#adminAgreementClausesContainer");
    if (!clausesContainer) { showMessage("Error", "Clauses container not found in HTML."); return; }
    ...
    clauseDiv.innerHTML = `<div class="form-group"><label>Heading (Clause ${newClauseIndex + 1}):</label><input type="text" class="clause-heading-input" placeholder="Enter clause heading"></div><div class="form-group"><label>Body:</label><textarea class="clause-body-textarea" rows="4" placeholder="Enter clause body. Use placeholders like {{participantName}}, {{workerName}}, {{serviceList}} where appropriate."></textarea></div><button class="btn-danger btn-small remove-clause-btn" data-index="${newClauseIndex}"><i class="fas fa-trash-alt"></i> Remove Clause</button><hr class="compact-hr">`;
...
async function loadShiftRequestsForUserDisplay() {
    ...
    try {
        ...
        if (querySnapshot.empty) {
            ...
            rqTblBody.innerHTML = ""; 
            querySnapshot.forEach(docSnap => {
                const req = docSnap.data(); const tr = rqTblBody.insertRow();
...
function displayUploadedFilesWizard() {
    ...
    } else { fileListDiv.textContent = 'No files selected.'; }
}
...
function openLogShiftModal() {
    ...
    if (logShiftModalEl) {
        ...
        if (supportTypeSelect) {
            ...
            if (profile && profile.authorizedServiceCodes && availableServices.length > 0) {
                ...
                availableServices.forEach(service => {
                    const opt = document.createElement('option'); opt.value = service.code;
        ...
        if (startTimeInput) {
            startTimeInput.value = ""; startTimeInput.dataset.value24 = ""; 
            startTimeInput.onclick = () => openCustomTimePicker(startTimeInput, null); 
        }
        const endTimeInput = $("#logShiftEndTime");
        if (endTimeInput) {
            endTimeInput.value = ""; endTimeInput.dataset.value24 = "";
    ...
    } else { showMessage("Error", "Log shift modal element not found."); }
}

function handleLogShiftTravelToggle() { 
    const kmFieldsContainer = $("#logShiftKmFieldsContainer");
    if (this.checked) { kmFieldsContainer.classList.remove('hide'); 
    } else { 
        kmFieldsContainer.classList.add('hide'); 
        $("#logShiftStartKm").value = ""; $("#logShiftEndKm").value = "";
...
function calculateLogShiftTravelKm() {
    ...
    if (endKm > startKm) { calculatedKmSpan.textContent = `${(endKm - startKm).toFixed(1)} Km`; } 
    else { calculatedKmSpan.textContent = "0.0 Km"; }
}

function updateRateFieldsVisibility(categoryType) {
    const container = $("#adminServiceRateFieldsContainer"); if(!container) return;
    container.innerHTML = ""; 
...
function renderAdminServicesTable() {
    const tbody = $("#adminServicesTable tbody"); if (!tbody) return;
    tbody.innerHTML = ""; 
    if (adminManagedServices.length === 0) {
        const tr = tbody.insertRow(); const td = tr.insertCell();
        td.colSpan = 6; td.textContent = "No NDIS services defined yet. Add services using the form above.";
        td.style.textAlign = "center"; return;
    }
    adminManagedServices.forEach(s => {
        ...
        actionsCell.innerHTML=`<button onclick="window.editAdminService('${s.id}')" class="btn-secondary btn-small" title="Edit Service"><i class="fas fa-edit"></i></button> <button onclick="window.deleteAdminService('${s.id}')" class="btn-danger btn-small" title="Delete Service"><i class="fas fa-trash-alt"></i></button>`;
    });
}

window.editAdminService = function(serviceId) {
    const serviceToEdit = adminManagedServices.find(item => item.id === serviceId);
    if (!serviceToEdit) { showMessage("Error", "Service not found for editing."); return; }
...
window.deleteAdminService = async function(serviceId) {
    ...
    showMessage("Confirm Delete", 
        `Are you sure you want to delete the service "${service.description} (${service.code})"? This cannot be undone.<br><br><div class='modal-actions' style='justify-content: center; margin-top: 15px;'><button onclick='window._confirmDeleteServiceFirestore("${serviceId}")' class='btn-danger'><i class="fas fa-trash-alt"></i> Yes, Delete</button><button class='btn-secondary' onclick='closeModal("messageModal")'><i class="fas fa-times"></i> No, Cancel</button></div>`);
};
...
window.saveAdminService = async function() {
    ...
    const rates = {}; let allRatesValid = true; let firstInvalidRateField = null;
    ...
    if (serviceCategoryType === SERVICE_CATEGORY_TYPES.CORE_STANDARD || serviceCategoryType === SERVICE_CATEGORY_TYPES.CORE_HIGH_INTENSITY) {
        RATE_CATEGORIES.forEach(cat => {
            ...
            if (input) { const valStr = input.value.trim(); if (valStr === "") { rates[cat] = 0; } else { const val = parseFloat(valStr); if (isNaN(val) || val < 0) { if(allRatesValid) firstInvalidRateField = input; allRatesValid = false; rates[cat] = 0; } else { rates[cat] = val; } }
        ...
        if (input) { const valStr = input.value.trim(); if (valStr === "") { rates.standardRate = 0; } else { const val = parseFloat(valStr); if (isNaN(val) || val < 0) { if(allRatesValid) firstInvalidRateField = input; allRatesValid = false; rates.standardRate = 0; } else { rates.standardRate = val; } }
        ...
        if (input) { const valStr = input.value.trim(); if (valStr === "") { rates.perKmRate = 0; } else { const val = parseFloat(valStr); if (isNaN(val) || val < 0) { if(allRatesValid) firstInvalidRateField = input; allRatesValid = false; rates.perKmRate = 0; } else { rates.perKmRate = val; } }
    ...
    if (!allRatesValid) {
        ...
        if(firstInvalidRateField) firstInvalidRateField.focus(); return;
    }
    if ((serviceCategoryType === SERVICE_CATEGORY_TYPES.CORE_STANDARD || serviceCategoryType === SERVICE_CATEGORY_TYPES.CORE_HIGH_INTENSITY) && (rates.weekday === undefined || rates.weekday <= 0)) {
        ...
        const weekdayRateField = $("#adminServiceRate_weekday"); if (weekdayRateField) weekdayRateField.focus(); return;
    }
     if (serviceCategoryType === SERVICE_CATEGORY_TYPES.TRAVEL_KM && (rates.perKmRate === undefined || rates.perKmRate <= 0)) {
        ...
        const perKmRateField = $("#adminServiceRate_perKmRate"); if (perKmRateField) perKmRateField.focus(); return;
    }
    const servicePayload = {
        code: serviceCode, description: serviceDescription, categoryType: serviceCategoryType,
        rates: rates, travelCode: serviceTravelCode, isActiveInAgreement: true 
    };
    ...
    hideLoading();
    if (success) {
        renderAdminServicesTable(); clearAdminServiceForm();    
...
function loadAdminAgreementCustomizations() {
    ...
    const clausesContainer = $("#adminAgreementClausesContainer"); if(!clausesContainer) return;
    clausesContainer.innerHTML = ""; 
    (agreementCustomData.clauses || []).forEach((clause, index) => {
        const clauseDiv = document.createElement('div'); clauseDiv.className = 'agreement-clause-editor';
        clauseDiv.innerHTML = `<div class="form-group"><label>Heading (Clause ${index + 1}):</label><input type="text" class="clause-heading-input" value="${clause.heading || ''}"></div><div class="form-group"><label>Body:</label><textarea class="clause-body-textarea" rows="4">${clause.body || ''}</textarea></div><button class="btn-danger btn-small remove-clause-btn" data-index="${index}"><i class="fas fa-trash-alt"></i> Remove</button><hr class="compact-hr">`;
        clausesContainer.appendChild(clauseDiv);
        clauseDiv.querySelector('.remove-clause-btn').addEventListener('click', function() {
            this.closest('.agreement-clause-editor').remove();
            clausesContainer.querySelectorAll('.agreement-clause-editor').forEach((editor, idx) => {
...
function renderAdminAgreementPreview() {
    const previewBox = $("#adminAgreementPreview"); if (!previewBox) return;
     if (typeof agreementCustomData !== 'object' || agreementCustomData === null) {
...
function populateAdminWorkerSelectorForAgreement() {
    const selector = $("#adminSelectWorkerForAgreement"); if (!selector) return;
...
async function loadServiceAgreement() {
    if (!currentUserId || !isFirebaseInitialized) {
        $("#agreementContentContainer").innerHTML = "<p><em>Error: User not logged in or Firebase not ready.</em></p>"; return;
    }
    if (typeof agreementCustomData !== 'object' || agreementCustomData === null || !agreementCustomData.clauses) {
        ...
        if (typeof agreementCustomData !== 'object' || agreementCustomData === null || !agreementCustomData.clauses) {
            $("#agreementContentContainer").innerHTML = "<p><em>Error: Agreement template data is missing and could not be loaded. Please configure it in the admin panel.</em></p>"; return;
        }
    }

    let workerProfileToUse; let workerName, workerAbn; let agreementDocPath; 
    if (profile.isAdmin && currentAgreementWorkerEmail) { 
        workerProfileToUse = accounts[currentAgreementWorkerEmail]?.profile;
        if (!workerProfileToUse) {
            $("#agreementContentContainer").innerHTML = "<p><em>Error: Selected worker profile not found.</em></p>"; return;
        }
        workerName = workerProfileToUse.name; workerAbn = workerProfileToUse.abn;
        ...
        workerProfileToUse = profile; workerName = profile.name; workerAbn = profile.abn;
        $("#agreementDynamicTitle").innerHTML = `<i class="fas fa-handshake"></i> Your Service Agreement`;
        agreementDocPath = `artifacts/${appId}/users/${currentUserId}/agreements/main`;
    } else { 
        $("#agreementContentContainer").innerHTML = "<p><em>Cannot determine whose agreement to load.</em></p>"; return;
    }
    const agreementContainer = $("#agreementContentContainer"); if (!agreementContainer) return;
    ...
    (agreementCustomData.clauses || []).forEach(clause => {
        ...
        } else { serviceListHtml += "<li>No specific services listed/authorized. General support will be provided as agreed.</li>"; }
        serviceListHtml += "</ul>";
    ...
    if (agreementInstanceData.participantSigUrl && sigPImg) sigPImg.src = agreementInstanceData.participantSigUrl; else if (sigPImg) sigPImg.src = "https://placehold.co/250x85/e8e8e8/666666?text=Signature+Area"; 
    ...
    if (agreementInstanceData.workerSigUrl && sigWImg) sigWImg.src = agreementInstanceData.workerSigUrl; else if (sigWImg) sigWImg.src = "https://placehold.co/250x85/e8e8e8/666666?text=Signature+Area";
    ...
    const agrChipEl = $("#agrChip"); const signBtnEl = $("#signBtn"); 
    const participantSignBtnEl = $("#participantSignBtn"); const pdfBtnEl = $("#pdfBtn");
    if (agreementInstanceData.workerSigned && agreementInstanceData.participantSigned) {
        ...
        if (signBtnEl) signBtnEl.classList.add("hide"); if (participantSignBtnEl) participantSignBtnEl.classList.add("hide");
        ...
        if (signBtnEl) signBtnEl.classList.remove("hide"); if (participantSignBtnEl) participantSignBtnEl.classList.remove("hide");
    }
...
async function generateAgreementPdf() {
    if (!currentUserId || !profile || !agreementCustomData) {
        showMessage("Error", "Required data for PDF generation is missing."); return;
    }
    ...
    if (profile.isAdmin && currentAgreementWorkerEmail) {
        ...
        if (selectedWorker) {
            workerProfileToUse = selectedWorker; workerName = selectedWorker.name ?? "Worker Name N/A";
            workerAbn = selectedWorker.abn ?? "Worker ABN N/A";
        } else { showMessage("Error", "Selected worker profile for PDF not found."); return; }
    }
    let agreementInstanceData = {
        workerSigUrl: $("#sigW")?.src, participantSigUrl: $("#sigP")?.src,
        workerSignDate: $("#dW")?.textContent, participantSignDate: $("#dP")?.textContent,
    ...
    let pdfHtml = `<style>body { font-family: 'Inter', sans-serif; font-size: 10pt; color: #333; } .pdf-agreement-container { padding: 20mm; } .pdf-agreement-header h1 { font-size: 18pt; text-align: center; margin-bottom: 5mm; color: #000; } .pdf-agreement-header h2 { font-size: 14pt; text-align: center; margin-bottom: 10mm; color: #222; } .pdf-clause h4 { font-size: 12pt; margin-top: 8mm; margin-bottom: 3mm; color: #111; border-bottom: 1px solid #eee; padding-bottom: 2mm;} .pdf-clause div { margin-bottom: 5mm; line-height: 1.5; text-align: justify; } .pdf-clause ul { padding-left: 20px; margin-bottom: 5mm; } .pdf-clause li { margin-bottom: 2mm; } .pdf-signatures { margin-top: 15mm; display: flex; justify-content: space-around; page-break-inside: avoid; } .pdf-signature-block { width: 45%; text-align: center; } .pdf-signature-block img { width: 150px; height: 50px; border: 1px dashed #ccc; margin-bottom: 5mm; background-color: #f9f9f9; object-fit: contain; } .pdf-signature-block p { margin: 2mm 0; font-size: 9pt; }</style><div class="pdf-agreement-container"><div class="pdf-agreement-header"><h1>${globalSettings.organizationName ?? 'Service Provider'}</h1> <h2>${agreementCustomData.overallTitle ?? "Service Agreement"}</h2></div>`;
    (agreementCustomData.clauses || []).forEach(clause => {
        let clauseBody = clause.body || "";
        clauseBody = clauseBody.replace(/{{participantName}}/g, globalSettings.participantName ?? "[Participant Name]")
                               .replace(/{{participantNdisNo}}/g, globalSettings.participantNdisNo ?? "[NDIS No]")
        ...
        } else { serviceListHtml += "<li>No specific services listed/authorized.</li>"; }
        serviceListHtml += "</ul>";
        clauseBody = clauseBody.replace(/{{serviceList}}/g, serviceListHtml);
        pdfHtml += `<div class="pdf-clause"><h4>${clause.heading || ""}</h4><div>${clauseBody.replace(/\n/g, '<br>')}</div></div>`;
    });
    pdfHtml += `<div class="pdf-signatures"><div class="pdf-signature-block"><p><strong>Participant</strong></p>${agreementInstanceData.participantSigUrl && agreementInstanceData.participantSigUrl !== "https://placehold.co/250x85/e8e8e8/666666?text=Signature+Area" ? `<img src="${agreementInstanceData.participantSigUrl}" alt="Participant Signature">` : '<div style="width:150px; height:50px; border:1px dashed #ccc; margin: 0 auto 5mm; display:flex; align-items:center; justify-content:center; font-size:8pt; color:#999;">Signature Area</div>'}<p>Date: ${agreementInstanceData.participantSignDate && agreementInstanceData.participantSignDate !== '___' ? agreementInstanceData.participantSignDate : '_____________________'}</p></div><div class="pdf-signature-block"><p><strong>Support Worker</strong></p>${agreementInstanceData.workerSigUrl && agreementInstanceData.workerSigUrl !== "https://placehold.co/250x85/e8e8e8/666666?text=Signature+Area" ? `<img src="${agreementInstanceData.workerSigUrl}" alt="Support Worker Signature">` : '<div style="width:150px; height:50px; border:1px dashed #ccc; margin: 0 auto 5mm; display:flex; align-items:center; justify-content:center; font-size:8pt; color:#999;">Signature Area</div>'}<p>Date: ${agreementInstanceData.workerSignDate && agreementInstanceData.workerSignDate !== '___' ? agreementInstanceData.workerSignDate : '_____________________'}</p></div></div></div>`;
    const tempDivAgreement = document.createElement("div");
    tempDivAgreement.style.position = "absolute"; tempDivAgreement.style.left = "-9999px"; tempDivAgreement.style.width = "210mm"; 
    tempDivAgreement.innerHTML = pdfHtml; document.body.appendChild(tempDivAgreement);
    ...
    const opt = {
        margin: [15, 15, 15, 15], filename: agreementPdfFilename, image: { type: 'jpeg', quality: 0.98 },
        html2canvas: { scale: 2, useCORS: true, logging: false, scrollY: -window.scrollY },
        jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' }
    };
    html2pdf().from(tempDivAgreement).set(opt).save().then(() => {
        showMessage("PDF Generated", "Service Agreement PDF has been downloaded."); tempDivAgreement.remove(); 
    }).catch(err => {
        console.error("Error generating agreement PDF:", err);
        logErrorToFirestore("generateAgreementPdf", err.message, err);
        showMessage("PDF Error", "Could not generate PDF: " + err.message); tempDivAgreement.remove();
    });
}
    ...
    if (filesListUl) {
        if (profile.files && profile.files.length > 0) {
            ...
            profile.files.forEach(file => {
                const li = document.createElement('li');
                li.innerHTML = `<a href="${file.url || '#'}" target="_blank" title="${file.url ? 'Open file' : 'File URL not available'}">${file.name || 'Unnamed File'}</a><button class="btn-danger btn-small" onclick="window.deleteProfileDocument('${file.name}', '${file.storagePath || ''}')" title="Delete ${file.name}"><i class="fas fa-trash-alt"></i></button>`;
                filesListUl.appendChild(li);
            });
        } else { filesListUl.innerHTML = "<li>No documents uploaded yet.</li>"; }
    }
}
window.deleteProfileDocument = async function(fileName, storagePath) {
    if (!currentUserId || !fsDb) { showMessage("Error", "User not logged in or database not ready."); return; }
    if (!fileName) { showMessage("Error", "File name not provided for deletion."); return; }
    showMessage("Confirm Delete", 
        `Are you sure you want to delete the document "${fileName}"? This action cannot be undone.<br><br><div class='modal-actions' style='justify-content: center; margin-top: 15px;'><button onclick='window._confirmDeleteProfileDocument("${fileName}", "${storagePath}")' class='btn-danger'><i class="fas fa-trash-alt"></i> Yes, Delete</button><button class='btn-secondary' onclick='closeModal("messageModal")'><i class="fas fa-times"></i> No, Cancel</button></div>`);
};
...
window._confirmDeleteProfileDocument = async function(fileName, storagePath) {
    ...
    if (storagePath && fbStorage) {
        ...
        try { await deleteObject(fileRef); console.log(`File deleted from Storage: ${storagePath}`);
        } catch (storageError) {
    ...
    } else { console.warn("Storage path not provided or Storage not initialized. Skipping cloud deletion for:", fileName); }
    try {
        const userProfileDocRef = doc(fsDb, `artifacts/${appId}/users/${currentUserId}/profile`, "details");
        ...
        } else { console.warn("File metadata not found in profile for deletion:", fileName, storagePath); }
        loadProfileData(); 
...
function enterPortal(isAdmin) {
    if (isAdmin) { setActive(location.hash || "#admin"); 
    } else { 
        if (globalSettings.portalType === 'organization' && (!profile.abn || !profile.bsb || !profile.acc || !profile.profileSetupComplete )) {
            openUserSetupWizard(); return; 
        }
...
function formatInvoiceNumber(num) { return `INV-${String(num).padStart(6, '0')}`; }
...
async function loadDraftInvoice() {
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            invTblBody.innerHTML = ""; addInvoiceRow(); 
        }
    } catch (error) {
...
function openCustomTimePicker(inputElement, callbackFn) {
    activeTimeInput = inputElement; timePickerCallback = callbackFn; 
    const picker = $("#customTimePicker");
    if (picker) {
        picker.classList.remove('hide'); picker.style.display = 'flex'; 
...
function updateTimePickerStepView() {
    const picker = $("#customTimePicker"); if (!picker) return;
    const stepLabel = $("#currentTimePickerStepLabel");
    const ampmStepDiv = $("#timePickerStepAmPm"); const hourStepDiv = $("#timePickerStepHour");
    const minuteStepDiv = $("#timePickerStepMinute"); const backButton = $("#timePickerBackButton");
    const setButton = $("#setTimeButton");
    if (ampmStepDiv) ampmStepDiv.classList.add('hide'); if (hourStepDiv) hourStepDiv.classList.add('hide');
    if (minuteStepDiv) minuteStepDiv.classList.add('hide'); if (backButton) backButton.classList.add('hide');
...
async function logout() {
  ...
  try {
    showLoading("Logging out..."); await fbSignOut(fbAuth);
  } catch (e) {
    console.error("Logout failed:", e); logErrorToFirestore("logout", e.message, e);
...
window.saveAdminPortalSettings = async function() {
    if (!isFirebaseInitialized || !(profile?.isAdmin)) {
        showMessage("Permission Denied", "You do not have permission to save portal settings."); return;
    }
    ...
    await saveGlobalSettingsToFirestore(); 
    hideLoading(); showMessage("Settings Saved", "Global portal settings have been updated successfully.");
    loadAdminPortalSettings(); setActive(location.hash); 
};

window.resetGlobalSettingsToDefaults = function() {
    if (!(profile?.isAdmin)) { showMessage("Permission Denied", "You do not have permission to reset settings."); return; }
    showMessage("Confirm Reset", 
        `Are you sure you want to reset all portal settings to their original defaults? This action cannot be undone.<br><br><div class='modal-actions' style='justify-content: center; margin-top: 15px;'><button onclick='window._confirmResetGlobalSettingsFirestore()' class='btn-danger'><i class="fas fa-undo"></i> Yes, Reset</button><button class='btn-secondary' onclick='closeModal("messageModal")'><i class="fas fa-times"></i> No, Cancel</button></div>`);
};
...
window._confirmResetGlobalSettingsFirestore = async function() {
    ...
    if (location.hash === "#admin") { 
        ...
        if (authScreenElement) authScreenElement.style.display = "flex"; 
        if (portalAppElement) portalAppElement.style.display = "none";
        if (profile && profile.isAdmin && !globalSettings.setupComplete) { openAdminSetupWizard(); }
    }
};
...
// Make sure all functions that might be called from HTML string onclicks are on window
// Most are already, but this is a safeguard for any missed ones.
// window.approveWorkerInFirestore is defined above with window.
// window.selectWorkerForAuth is defined above with window.

let invoiceItemCounter = 0; 

function addInvoiceRow(itemData = null, isLoadingFromDraft = false) {
    const tbody = $("#invTbl tbody"); if (!tbody) return;
    ...
    const dateCell = tr.insertCell(); const dateInput = document.createElement('input');
    dateInput.type = 'date'; dateInput.id = `itemDate${rowIndex}`; dateInput.className = 'invoice-input-condensed';
    dateInput.value = itemData?.date || new Date().toISOString().split('T')[0]; 
    dateInput.onchange = calculateInvoiceTotals; 
    dateCell.appendChild(dateInput);
    const codeCellPrint = tr.insertCell(); codeCellPrint.className = 'column-code print-only pdf-show'; 
    const codePrintSpan = document.createElement('span'); codePrintSpan.id = `itemCodePrint${rowIndex}`;
    codePrintSpan.className = 'code-print-value'; codePrintSpan.textContent = itemData?.serviceCode || "";
    codeCellPrint.appendChild(codePrintSpan);
    const descCell = tr.insertCell(); const descSelect = document.createElement('select');
    descSelect.id = `itemDesc${rowIndex}`; descSelect.className = 'invoice-input-condensed description-select'; 
    descSelect.innerHTML = `<option value="">-- Select Service --</option>`; 
    const availableServices = adminManagedServices.filter(s => profile.authorizedServiceCodes?.includes(s.code));
    availableServices.forEach(service => {
        const opt = document.createElement('option'); opt.value = service.code;
        opt.textContent = `${service.description} (${service.code})`;
    ...
    descPrintSpan.id = `itemDescPrint${rowIndex}`; descPrintSpan.className = 'description-print-value'; 
    ...
    const startCell = tr.insertCell(); const startTimeInput = document.createElement('input');
    startTimeInput.type = 'text'; startTimeInput.id = `itemStart${rowIndex}`;
    startTimeInput.className = 'custom-time-input invoice-input-condensed'; startTimeInput.readOnly = true; 
    startTimeInput.placeholder = "Select Time"; startTimeInput.value = itemData?.startTime ? formatTime12Hour(itemData.startTime) : "";
    ...
    const endCell = tr.insertCell(); const endTimeInput = document.createElement('input');
    endTimeInput.type = 'text'; endTimeInput.id = `itemEnd${rowIndex}`;
    endTimeInput.className = 'custom-time-input invoice-input-condensed'; endTimeInput.readOnly = true;
    endTimeInput.placeholder = "Select Time"; endTimeInput.value = itemData?.endTime ? formatTime12Hour(itemData.endTime) : "";
    ...
    const rateTypeCellPrint = tr.insertCell(); rateTypeCellPrint.className = 'column-rate-type print-only pdf-show';
    const rateTypePrintSpan = document.createElement('span'); rateTypePrintSpan.id = `itemRateTypePrint${rowIndex}`;
    ...
    const rateUnitCellPrint = tr.insertCell(); rateUnitCellPrint.className = 'print-only-column pdf-show'; 
    rateUnitCellPrint.id = `itemRateUnitPrint${rowIndex}`; rateUnitCellPrint.textContent = "$0.00"; 
    descSelect.onchange = (e) => {
        ...
        if(selectedService) {
            if (selectedService.categoryType === SERVICE_CATEGORY_TYPES.TRAVEL_KM) { rateForPrintOnChange = selectedService.rates?.perKmRate || 0; } 
            else if (selectedService.categoryType === SERVICE_CATEGORY_TYPES.CORE_STANDARD || selectedService.categoryType === SERVICE_CATEGORY_TYPES.CORE_HIGH_INTENSITY) { rateForPrintOnChange = selectedService.rates?.[rt] || selectedService.rates?.weekday || 0; } 
            else if (selectedService.categoryType === SERVICE_CATEGORY_TYPES.CAPACITY_THERAPY_STD || selectedService.categoryType === SERVICE_CATEGORY_TYPES.CAPACITY_SPECIALIST || selectedService.categoryType === SERVICE_CATEGORY_TYPES.OTHER_FLAT_RATE) { rateForPrintOnChange = selectedService.rates?.standardRate || 0; }
        }
    ...
    const hoursKmCell = tr.insertCell(); hoursKmCell.id = `itemHoursKm${rowIndex}`;
    hoursKmCell.textContent = itemData?.hoursOrKm?.toFixed(2) || "0.00";
    const travelInputCell = tr.insertCell(); travelInputCell.className = 'no-print pdf-hide'; 
    const travelKmInput = document.createElement('input'); travelKmInput.type = 'number';
    travelKmInput.id = `itemTravel${rowIndex}`; travelKmInput.className = 'invoice-input-condensed';
    travelKmInput.placeholder = "Km"; travelKmInput.step = "0.1"; travelKmInput.min = "0";
    travelKmInput.value = itemData?.travelKmInput || ""; travelKmInput.oninput = calculateInvoiceTotals;
    ...
    const claimTravelCell = tr.insertCell(); claimTravelCell.className = 'no-print pdf-hide';
    const claimTravelLabel = document.createElement('label'); claimTravelLabel.className = 'chk no-margin km-claim-toggle'; 
    const claimTravelCheckbox = document.createElement('input'); claimTravelCheckbox.type = 'checkbox';
    claimTravelCheckbox.id = `itemClaimTravel${rowIndex}`; claimTravelCheckbox.checked = itemData?.claimTravel || false;
    claimTravelCheckbox.onchange = calculateInvoiceTotals; claimTravelLabel.appendChild(claimTravelCheckbox);
    ...
    const totalCell = tr.insertCell(); totalCell.id = `itemTotal${rowIndex}`;
    totalCell.textContent = itemData?.total ? `$${itemData.total.toFixed(2)}` : "$0.00";
    const actionsCell = tr.insertCell(); actionsCell.className = 'no-print pdf-hide';
    const deleteBtn = document.createElement('button'); deleteBtn.innerHTML = '<i class="fas fa-trash-alt"></i>';
    deleteBtn.className = 'btn-danger btn-small delete-row-btn'; deleteBtn.title = "Delete Row";
    deleteBtn.onclick = () => {
        tr.remove();
...
function calculateInvoiceTotals() {
    let subtotal = 0; const rows = $$("#invTbl tbody tr");
    rows.forEach((row) => {
        ...
        const hoursKmCell = row.cells[8]; const totalCell = row.cells[10];   
        ...
        if (!profile.isAdmin && profile.authorizedServiceCodes && !profile.authorizedServiceCodes.includes(serviceCode) && serviceCode !== "") {
            ...
            hoursKmCell.textContent = "0.00"; return; 
        }
        const service = adminManagedServices.find(s => s.code === serviceCode);
        let itemTotal = 0; let hours = 0; let km = 0; let rateForPrint = 0; 
        if (service) {
            const itemDate = dateInput?.value; const startTime = startTimeInput?.dataset.value24;
            ...
            if (service.categoryType === SERVICE_CATEGORY_TYPES.TRAVEL_KM) {
                km = parseFloat(travelKmInput?.value) || 0; hoursKmCell.textContent = km.toFixed(2);
                const rate = service.rates?.perKmRate || 0; itemTotal = km * rate; rateForPrint = rate;
                if(rateTypePrintSpan) rateTypePrintSpan.textContent = "Travel"; 
            } else { 
                hours = calculateHours(startTime, endTime); hoursKmCell.textContent = hours.toFixed(2);
    ...
    $("#sub").textContent = `$${subtotal.toFixed(2)}`; let gstAmount = 0;
    if (profile.gstRegistered) { 
        gstAmount = subtotal * 0.10; 
        $("#gst").textContent = `$${gstAmount.toFixed(2)}`; $("#gstRow").style.display = 'flex'; 
    } else {
        $("#gst").textContent = "$0.00"; $("#gstRow").style.display = 'none'; 
    }
...
// Ensure functions called from HTML string onclicks are globally available
// Many are already assigned to window, but this is a check.
// Example: window.approveWorkerInFirestore is assigned where defined.
// window.selectWorkerForAuth is assigned where defined.
// window.editAdminService, window.deleteAdminService, window._confirmDeleteServiceFirestore are assigned where defined.
// window.modalLogin, window.modalRegister are assigned where defined.
// window.saveSig, window.wizNext, window.wizPrev, window.wizFinish etc.
